...
async def test_build_and_deploy_without_trust(ops_test: OpsTest, kyuubi_charm, charm_versions):
    ...
    # Assert that intgration hub is in active state while Kyuubi is blocked due to insufficient permissions
    assert check_status(ops_test.model.applications[APP_NAME], Status.INSUFFICIENT_CLUSTER_PERMISSIONS.value)
    assert (
...
async def test_provide_clusterwide_trust_permissions(ops_test):
    ...
    assert check_status(ops_test.model.applications[APP_NAME], Status.MISSING_OBJECT_STORAGE_BACKEND.value)
